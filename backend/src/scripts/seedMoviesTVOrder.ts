...
const seedMoviesTVOrder = async () => {
  try {
    ...
    console.log('🌱 Adding Movies and TV Shows Order Questions...');

    const projectRoot = path.resolve(process.cwd(), '..');
    const questionsFilePath = path.join(projectRoot, 'questions safe place', 'moviestvshows-order.txt');
    ...
    console.log(`✅ تم إضافة ${questionsToInsert.length} سؤال عن الأفلام والمسلسلات (ترتيب)!`);
    process.exit(0);
  } catch (error) {
